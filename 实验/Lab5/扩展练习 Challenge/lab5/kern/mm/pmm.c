    ...
    bool share)
{
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
            ...
            // ============ LAB5 Challenge Code Start ============
            if (share) {
                // COW 逻辑：
                // 1. 如果原页面是可写的，需要将其改为只读，以便父进程写入时也能触发 COW
                // 注意：只有当页面本身是可写的时候才需要操作，如果本来就是只读（如代码段），则保持原样
                if (perm & PTE_W) {
                    perm &= ~PTE_W;           // 清除权限变量中的写位
                    *ptep &= ~PTE_W;          // 【关键】修改父进程的页表项为只读
                    // 修改了页表项权限，必须刷新 TLB，否则 CPU 缓存的 TLB 仍认为是可写的
                    tlb_invalidate(from, start);
                }

                // 2. 建立子进程的映射，使用修改后的 perm (只读)
                // page_insert 会自动增加 page 的引用计数 (page_ref_inc)
                ret = page_insert(to, page, start, perm);
            }
            else {
                // ============ 组员完成的练习2代码 (Deep Copy) ============
