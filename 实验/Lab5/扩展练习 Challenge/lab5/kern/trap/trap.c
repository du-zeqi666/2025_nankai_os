...
void exception_handler(struct trapframe* tf)
{
    ...
    case CAUSE_STORE_ACCESS:
        ...
        // ============ 修改重点：缺页异常处理 ============
        // 2313508: 处理 Page Fault，而不是简单的打印
        // 注意：这里合并了 Fetch, Load, Store 三种缺页异常
    case CAUSE_FETCH_PAGE_FAULT:
    case CAUSE_LOAD_PAGE_FAULT:
    case CAUSE_STORE_PAGE_FAULT:
        // tf->tval (即 stval 寄存器) 包含了出错的虚拟地址
        if ((ret = do_pgfault(current->mm, tf->cause, tf->tval)) != 0)
        {
            ...
            {
                // 如果 do_pgfault 返回失败，说明无法分配内存或权限错误，杀死进程
    ...
    case CAUSE_MACHINE_ECALL:
        ...
        // !!! 注意：这里原来那三个 printf 的 case 已经被删除了 !!!
        // 如果你不删掉它们，就会报错 duplicate case
