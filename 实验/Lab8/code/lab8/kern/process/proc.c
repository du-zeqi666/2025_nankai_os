...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2313508 : (update LAB6 steps)
        /*
         * below fields(add in LAB6) in proc_struct need to be initialized
        ...
        proc->filesp = NULL;  //LAB8: initialize filesp
    }
    return proc;
}
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // LAB8:EXERCISE2 2313508  HINT:how to copy the fs in parent's proc_struct?
    ...
    // LAB8 fix: manually copy pages if pmm.c copy_range is empty
    if (!(clone_flags & CLONE_VM)) {
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    for (int i = 0; i < argc; i++) {
        ...
        char *src = kargv[i];
        while (len > 0) {
            struct Page *p = get_page(mm->pgdir, dst, NULL);
            assert(p != NULL);
            uintptr_t kva = (uintptr_t)page2kva(p);
            off_t off = dst % PGSIZE;
            size_t copylen = PGSIZE - off;
            if (copylen > len) copylen = len;
            memcpy((void *)(kva + off), src, copylen);
            dst += copylen;
            src += copylen;
            len -= copylen;
        }
